...
class QuantSaleOrderWizard(models.TransientModel):
    ...
    team_id = fields.Many2one("crm.team", string="Sales Channel", required=True)
    ...
    def default_get(self, fields):
        ...
        if source_location.usage != "internal":
            raise UserError(
                _(
                    "Please select quants that are in the internal "
        ...
        if order_line_list:
            ...
            raise UserError(
                _(
                    "There is at least one active sales order "
                    "that uses the product of a selected quant: "
                    "%s"
                )
                % error_msg
            )
        if any(self.env.user.company_id.id != q.company_id.id for q in quant_ids):
            raise UserError(
                _(
                    "You cannot create sales order from stock "
