...
class TestSaleOrderImport(common.TransactionCase):
    ...
    def test_01_get_order_dict_flow(self):
        ...
        wizard_sale = self.env["import.sale"].create(
            {
                ...
                "customer_invoice_journal_id": self.env["account.journal"]
                .search(
                ...
                "asynchronous": False,
                "input_file": base64.encodebytes(generated_file),
        ...
        # contains the `ids` of data.import.log object.
        res_domain = safe_eval(res.get("domain", {}))[0][2]

        # Browse the data.import.log object ids which return in res_domain
        import_data_log = self.env["data.import.log"].browse(res_domain)
        sale_order = import_data_log.sale_order_ids
        ...
        # San Francisco,Test Note,Free delivery charges,Sales,+1234567890

        # Compares the values with test_regular_sale_order.csv file.
        self.assertEqual(sale_order[0].partner_id, self.partner_01)
        self.assertEqual(sale_order[1].partner_id, self.partner_02)
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            self.partner_01.property_payment_term_id,
    ...
    def test_02_update_error_log(self):
        ...
        # Search the data import log which is created by `_update_data_import_log`
        data_import_log_id = self.env["data.import.log"].search(
            [("state", "=", "failed")]
        )
        ...
        self.assertEqual(data_import_log_id.error_ids[0].row_no, 2)

        # Compare the group number from the error sale order csv file..
        self.assertEqual(data_import_log_id.error_ids[0].reference, "1")
        ...
        # Compare the row no from the error sale order csv file..
        self.assertEqual(data_import_log_id.error_ids[1].row_no, 3)

        # Compare the group number from the error sale order csv file..
        self.assertEqual(data_import_log_id.error_ids[1].reference, "2")

        # Compare the length of error log lines records.
        self.assertEqual(
            len(data_import_log_id.error_ids), 2, "Check number of records for log line"
        )
